...
export function importOffers({ rows, mappingData, now = new Date() }) {
  ...
  if (!Number.isInteger(advertiserId) || advertiserId < 1) {
    throw new Error('Advertiser-ID fehlt. Import erst nach Programmfreigabe und bestätigter Awin-ID ausführen.');
  }
  if (!/^[a-z0-9-]+$/.test(mappingData.merchant?.id ?? '') || String(mappingData.merchant?.name ?? '').length < 2) {
    throw new Error('Mapping benötigt eine gültige Händler-ID und einen Händlernamen.');
  }
  ...
  for (const row of rows) {
    ...
    const offer = {
      offerId: `${mappingData.merchant.id}-${slug(merchantProductId)}`,
...
export function mergeOfferDocuments(existing, incoming) {
  if (!existing) return incoming;
  const incomingAdvertisers = new Set((incoming.sources ?? []).map((source) => `${source.network}:${source.advertiserId}`));
  const sources = [
    ...(existing.sources ?? []).filter((source) => !incomingAdvertisers.has(`${source.network}:${source.advertiserId}`)),
    ...(incoming.sources ?? [])
  ].sort((left, right) => `${left.network}:${left.advertiserId}`.localeCompare(`${right.network}:${right.advertiserId}`));
  const offers = [
    ...(existing.offers ?? []).filter((offer) => !incomingAdvertisers.has(`${offer.network?.id}:${offer.network?.advertiserId}`)),
    ...(incoming.offers ?? [])
  ].sort((left, right) =>
    left.productId.localeCompare(right.productId) ||
    left.merchant.name.localeCompare(right.merchant.name, 'de') ||
...
export function runCli(argv = process.argv.slice(2)) {
  ...
  if (!args.input || !args.mapping || !args.output) {
    console.error('Aufruf: node tools/import-awin-offers.mjs --input <feed.csv|feed.csv.gz> --mapping <mapping.json> --output <offers.json> [--merge <bestehende-offers.json>]');
    process.exit(2);
  }
  ...
  const imported = importOffers({ rows, mappingData });
  let document = imported.document;
  const report = imported.report;
  if (args.merge) {
    const mergeFile = path.resolve(root, args.merge);
    const existing = fs.existsSync(mergeFile) ? JSON.parse(fs.readFileSync(mergeFile, 'utf8')) : null;
    document = mergeOfferDocuments(existing, document);
  }
