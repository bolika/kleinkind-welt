...
const allowedStatuses = new Set(['blocked_source_conflict', 'researched_pending_safety_gate', 'ready_for_catalog_modeling']);
...
for (const record of batch.records ?? []) {
  ...
  if (!candidateById.has(record.canonicalModelId)) errors.push(`${record.canonicalModelId}: kein korrespondierender Backlog-Kandidat`);
  if (publishedIds.has(record.canonicalModelId)) errors.push(`${record.canonicalModelId}: bereits veröffentlicht und darf nicht im Research-Batch bleiben`);
  ...
  if (record.status === 'researched_pending_safety_gate' || record.status === 'ready_for_catalog_modeling') {
    for (const field of [
      'officialModelName',
    ...
    if (candidateById.get(record.canonicalModelId)?.status !== record.status) {
...
console.log(`Research-Batch-Gate bestanden: ${researched} vollständig recherchiert, ${blocked} transparent blockiert.`);
